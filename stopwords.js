/*
	Remove stop words (English) from a phrase or text
	Long Stopword List , http://www.ranks.nl/resources/stopwords.html.
*/

var isStopWord = function(word)
{
	var regex = new RegExp("\\b"+word+"\\b","i");
	if(stopWords.search(regex) < 0)	return false;
	else 	return true;	
}
...
exports.removeStopWords = function(text){
	...
	var tokens = removed_punc_text.split(/\s+/);//split on multiple spaces
	var words = [];
	
	for	(idx = 0; idx < tokens.length; idx++){
		if(!isStopWord(tokens[idx]))	words.push(tokens[idx]);
	}
